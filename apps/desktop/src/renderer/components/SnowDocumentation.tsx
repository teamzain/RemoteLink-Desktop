...
const categories: Category[] = [
  {
    ...
    bg: 'bg-blue-50/50',
    items: [
      { title: 'System Requirements', content: 'Remote 365 requires Windows 10+, macOS 12+, or Ubuntu 20.04+. Minimum 4GB RAM and a stable internet connection (5 Mbps+). For hosting, port 443 should be open or a relay will be used automatically.' },
      ...
      { title: 'Authentication Flows', content: 'Remote 365 uses JWT-based auth with short-lived access tokens (15 min) and long-lived refresh tokens (7 days). Tokens are stored securely in the system keychain on desktop. Web sessions fall back to localStorage.' },
    ]
  },
  {
    ...
    items: [
      ...
      { title: 'ICE/STUN/TURN Guides', content: 'Remote 365 first attempts a direct P2P connection using STUN. If NAT traversal fails (symmetric NAT), it falls back to TURN relay automatically. You can inspect your connection type in the session overlay (Ctrl+Shift+I).' },
...
const apiRefs = [
  { title: 'Command Line Interface', sub: 'Control nodes via terminal', icon: Terminal, content: 'Run `Remote 365 --help` to see all CLI commands. Key commands: `Remote 365 host` (start hosting), `Remote 365 connect <key>` (connect to a node), `Remote 365 status` (show active sessions), `Remote 365 logout` (clear credentials).' },
          ...
          {!customGuideOpen && !guideSent ? (
            ...
            <div className="w-full space-y-4">
              ...
              <textarea
                ...
                placeholder="E.g. We need a guide for deploying Remote 365 across 500 remote Windows workstations with SSO..."
