...
export const SnowSupport: React.FC = () => {
  const commonTopics = [
    ...
    { title: 'Identity & Access Keys', icon: ShieldCheck, color: 'text-emerald-500', bg: 'bg-emerald-50' },
    { title: 'P2P Signaling Over Relays', icon: Zap, color: 'text-amber-500', bg: 'bg-amber-50' }
  ];
  ...
  return (
    <div className="max-w-5xl mx-auto flex flex-col gap-10 animate-in fade-in slide-in-from-bottom-8 duration-700 font-inter pb-20">
      ...
      {/* Support Hero */}
      <div className="bg-[#1C1C1C] rounded-[40px] p-12 text-white relative overflow-hidden shadow-xl border border-[rgba(28,28,28,0.04)]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
         {commonTopics.map((topic, i) => (
           <div key={i} className="bg-white rounded-[24px] border border-[rgba(28,28,28,0.06)] p-6 shadow-sm hover:border-[rgba(28,28,28,0.2)] transition-all cursor-pointer group border border-[rgba(28,28,28,0.04)]">
      ...
      <div className="flex flex-col md:flex-row gap-8">
         ...
         <div className="flex-[2] bg-white rounded-[32px] border border-[rgba(28,28,28,0.06)] p-8 shadow-sm border border-[rgba(28,28,28,0.04)]">
            ...
            <div className="space-y-4">
               {tickets.map((ticket, i) => (
                 <div key={i} className="flex items-center justify-between p-4 bg-[#F9F9FA] rounded-[24px] border border-[rgba(28,28,28,0.02)] hover:border-[rgba(28,28,28,0.1)] transition-all cursor-pointer group border border-[rgba(28,28,28,0.04)]">
                    <div className="flex items-center gap-4">
                       <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center text-[rgba(28,28,28,0.4)] group-hover:text-black shadow-sm border border-[rgba(28,28,28,0.04)]">
                    ...
                    <div className="flex items-center gap-4">
                       <span className={`px-2 py-0.5 rounded-lg text-[9px] font-bold ${ticket.status === 'Closed' ? 'bg-slate-200 text-slate-500' : 'bg-emerald-50 text-emerald-500 border border-emerald-100'}`}>
                          {ticket.status}
                       </span>
         ...
         <div className="flex-1 space-y-4">
            <div className="bg-white rounded-[32px] border border-[rgba(28,28,28,0.06)] p-8 shadow-sm flex flex-col items-center text-center border border-[rgba(28,28,28,0.04)]">
            ...
            <div className="bg-white rounded-[32px] border border-[rgba(28,28,28,0.06)] p-8 shadow-sm flex flex-col items-center text-center border border-[rgba(28,28,28,0.04)]">
               <div className="w-16 h-16 bg-emerald-50 rounded-[20px] text-emerald-500 flex items-center justify-center mb-6">
                 <Mail size={32} />
               </div>
