...
class DXGICapture {
public:
    DXGICapture() : device(nullptr), context(nullptr), duplication(nullptr), stagingTexture(nullptr) {}
    ~DXGICapture() { Cleanup(); }
    ...
    Napi::Value Capture(const Napi::CallbackInfo& info) {
        ...
        if (!Initialize()) {
            Napi::Error::New(env, "Failed to initialize DXGI").ThrowAsJavaScriptException();
            return env.Null();
        }
        ...
        hr = desktopResource->QueryInterface(__uuidof(ID3D11Texture2D), (void**)&acquireTexture);
        ...
        if (stagingTexture) {
            ...
            if (currentStagingDesc.Width != desc.Width || currentStagingDesc.Height != desc.Height) {
        ...
        if (mapped.RowPitch == targetPitch) {
            ...
            for (unsigned int y = 0; y < desc.Height; ++y) {
                memcpy(dest + (y * targetPitch), src + (y * mapped.RowPitch), targetPitch);
            }
        }
